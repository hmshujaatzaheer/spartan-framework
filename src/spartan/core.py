...
class SPARTAN:
    ...
    def batch_process(
        ...
        """
        return [
            self.process(query, num_samples=num_samples)
    ...
    def evaluate_defense(
        ...
        tpr_at_fpr_01 = compute_tpr_at_fpr(
            ground_truth_labels, risk_scores, fpr_threshold=0.01
        )
        
        # Accuracy retention (mock - would need actual labels)
        avg_risk_reduction = np.mean([
            1 - r.risk_score for r in results if r.defense_applied
        ]) if any(r.defense_applied for r in results) else 0.0
