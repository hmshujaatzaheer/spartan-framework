...
from spartan.config import SPARTANConfig, RAASConfig
...
class RAAS:
    ...
    def sanitize(
        ...
        if (risk_analysis.prm_leakage > self._thresholds["prm"] and 
            reasoning_steps is not None):
        ...
        if (risk_analysis.vote_leakage > self._thresholds["vote"] and
            vote_distribution is not None):
        ...
        if (risk_analysis.mcts_leakage > self._thresholds["mcts"] and
            mcts_tree is not None):
    ...
    def _compute_epsilon(
        ...
        epsilon = (
            self._epsilon_min + 
            (self._epsilon_max - self._epsilon_min) * 
            risk_score * 
            importance_weight
        )
