...
from spartan.config import SPARTANConfig, RPPOConfig
...
class RPPO:
    ...
    def _gradient_refinement(
        ...
        if len(set(defense_intensities)) > 1:
            ...
            if not np.isnan(grad_epsilon):
                ...
                self._current_params["epsilon_max"] = float(
                    np.clip(new_epsilon_max, 0.1, 0.9)
                )
    ...
    def get_optimal_params(self) -> Optional[RPPOResult]:
        ...
        best_params.update({
            k: v for k, v in self._current_params.items()
            if k in ["epsilon_min", "epsilon_max"]
        })
